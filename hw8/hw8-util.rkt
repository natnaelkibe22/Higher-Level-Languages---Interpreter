...
(define (expect-expr1 e name)
  (when (not (j:expression? e)) (error "Expecting an expression, but got " e))
  e)
(define (expect-expr2 e1 e2 name)
  (when (not (j:expression? e1)) (error "Expecting an expression, but got " e1))
  (when (not (j:expression? e2)) (error "Expecting an expression, but got " e2))
  (values e1 e2))
...
(struct j:lambda (params body) #:transparent
  ;; We use guards to enforce show better error messages
  #:guard
  (lambda (p b n)
    (when (not (andmap j:variable? p))
      (error "Expecting all parameters to be variables, but got " p))
    (when (not (j:expression? b)) (error "Expecting an expression, but got " b))
    (values p b)))
(struct j:variable (name) #:transparent
  ...
  (lambda (x n)
    (when (not (symbol? x))
      (error "expecting a symbol, but got " x))
    x))
(struct j:apply (func args) #:transparent)
;; Object-related operations
(struct j:object (data) #:transparent #:guard
  (lambda (ht n)
    (unless (and (hash? ht) (andmap k:string? (hash-keys ht)) (andmap j:expression? (hash-values ht)))
      (error "Expecting a hash tables from strings to expressions, but got " ht))
    ht))
(struct j:get (obj field) #:transparent #:guard expect-expr2)
(define/contract (j:set/g obj field arg n)
  (-> j:expression? k:string? j:expression? symbol? (values j:expression? k:string? j:expression?))
  (values obj field arg))
(struct j:set (obj field arg) #:transparent #:guard j:set/g)
;; Heap-related operations
(struct j:alloc (value) #:transparent #:guard expect-expr1)
(struct j:deref (value) #:transparent #:guard expect-expr1)
(struct j:assign (ref value) #:transparent #:guard expect-expr2)
...
(struct s:load (obj field) #:transparent)
(struct s:assign (obj field arg) #:transparent)
(struct s:function (params body) #:transparent)
(struct s:invoke (obj meth args) #:transparent)
(struct s:apply (func args) #:transparent)
(struct s:new (constr args) #:transparent)
(struct s:class (parent methods) #:transparent)
(struct s:let (name body kont) #:transparent)
